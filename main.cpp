...
{
    Surface surface(W, H);
    Game_object::_surface = &surface;
    Quadtree quadtree;
    Event event;
    Mouse mouse;
    Node test_node;
    bool mouse_button_up = true;
    int add_object_delay = 0;
    int space_bar_delay = 0;
    bool show_quadrants = true;
    for (int i = 0; i < 100; ++i)
    {
        quadtree.add_new_object(new Game_object(rand() % W, rand() % (H - 50) + 50));
    }
    while (1)
    {
        ...
        {
            Game_object::inc_radius();
        }
        if (keypressed[DOWNARROW])
        {
            Game_object::dec_radius();
        }
        if (keypressed[SPACE] && space_bar_delay > 25)
        {
        ...
        {
            if (mouse_left() && mouse_button_up && add_object_delay > 25)
            {
                quadtree.add_new_object(new Game_object(mouse_x(), mouse_y()));
                add_object_delay = 0;
            }
        ...
        //quadtree.regular_collision_check();
        quadtree.quadtree_collision_check();
        surface.fill(BLACK);
        surface.lock();
        quadtree.draw(show_quadrants);
        surface.unlock();
        surface.flip();
        delay(10);
        ++add_object_delay;
